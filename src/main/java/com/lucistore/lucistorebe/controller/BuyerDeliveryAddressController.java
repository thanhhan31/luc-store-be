...
public class BuyerDeliveryAddressController {
	...
	public ResponseEntity<?> getAll(@AuthenticationPrincipal UserDetailsImpl<Buyer> buyer) {
		return ResponseEntity.ok(buyerDeliveryAddressService.getAllByIdBuyer(buyer.getUser().getId(), false));
	}
	...
	@DeleteMapping("/{id}")
	public ResponseEntity<?> delete(@PathVariable Long id, @AuthenticationPrincipal UserDetailsImpl<Buyer> buyer) {
